...
const RoutePath = ({systems}) => {
  const securityClass = sec => sec > 0.5 ? 'HS' : sec > 0 ? 'LS' : 'NS'
  const short = systems.reduce((memo, system) => {
    const secClass = securityClass(system.sec)
  ...
  return <div>
    { short.systems.map(s => {
      console.log(s.worstSec)
      return <div
        key={`${s.class}:${s.jumps}:${s.systems[0].name}-${s.systems.slice().pop().name}`}
      >
        <div
          className={`sec-${s.worstSec.toString().slice(0,3).replace('.', '-')}`}
          style={{
            width: 10,
            height: 10
        }} />
